...
/*   Updated: 2017/12/21 14:57:33 by lesanche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		line_len(t_list *begin_list)
{
	t_list	*elem;
	int		len;
	int		i;
...
int		get_next_line(const int fd, char **line)
{
	...
	if ((*line = (char*)malloc((line_len(b_list) + 1) * sizeof(char))) == NULL)
		return (0);
	*line[0] = '\0';
	el = b_list;
	while (el)
	{
		ft_strncat(*line, el->content, line_len(b_list) - ft_strlen(*line));
		el = el->next;
	}
	...
	if ((nb < BUFF_SIZE) && (ft_strlen(*line) == 0) && c_in_str(buf, '\n') == 1)
		return (0);
	return (1);
}
